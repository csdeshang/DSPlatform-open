<template>
    <el-dialog v-model="dialogVisible" :title="popTitle" width="800px" :destroy-on-close="true">
        <el-form :model="formData" label-width="180px" ref="formRef" :rules="formRules" v-loading="loading">
            ...
            <el-form-item label="商户ID" prop="config.ebusiness_id">
                <el-input v-model="formData.config.ebusiness_id" placeholder="请输入快递鸟商户ID" />
            </el-form-item>

            <el-form-item label="APP密钥" prop="config.app_key">
                <el-input v-model="formData.config.app_key" placeholder="请输入快递鸟APP密钥" type="password" show-password />
            </el-form-item>
            ...
            <template>
                <el-alert type="info" :closable="false" show-icon>
                    <p>使用提示：</p>
                    <p>1. 请先到快递鸟官网（https://www.kdniao.com/）注册账号并申请API权限</p>
                    <p>2. 获取商户ID和APP密钥后填入上方配置</p>
                    <p>3. 快递鸟支持1500+快递公司，覆盖面广</p>
...
const formRules = computed(() => {
    return {
        config: {
            ebusiness_id: [
                { required: true, message: '请输入快递鸟商户ID', trigger: 'blur' }
            ],
            app_key: [
                { required: true, message: '请输入快递鸟APP密钥', trigger: 'blur' }
            ],
            api_type: [
...
const setDialogData = async (row: any = null) => {
    ...
    try {
        ...
        // 获取服务商详细信息
        const { data } = await getExpressProviderInfo({ provider: row.provider });
