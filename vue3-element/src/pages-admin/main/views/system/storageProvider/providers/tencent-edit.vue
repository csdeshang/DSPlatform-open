<template>
    <el-dialog v-model="dialogVisible" :title="popTitle" width="800px" :destroy-on-close="true">
        ...
        <el-form :model="formData" label-width="180px" ref="formRef" :rules="formRules" v-loading="loading">
            ...
            <el-form-item label="Accesskey" prop="config.access_key_id">
                <el-input v-model="formData.config.access_key_id" placeholder="请输入Accesskey" />
            </el-form-item>
            ...
            <el-form-item label="endpoint" prop="config.endpoint">
                <el-input v-model="formData.config.endpoint" placeholder="请输入 endpoint" />
                <div>
...
import { updateStorageProviderConfig, getStorageProviderInfo } from '@/pages-admin/main/api/system/sysStorageProvider'

const dialogVisible = ref(false);
const loading = ref(false);
let popTitle = ref('编辑存储服务商');
...
const handleSubmit = async () => {
    ...
    try {
        ...
        const res = await updateStorageProviderConfig(updateData);

        ElMessage.success('更新存储服务商配置成功');
...
const setDialogData = async (row: any = null) => {
    if (!row || !row.provider) {
        ElMessage.warning('请选择要编辑的存储服务商');
        return;
    }
    ...
    try {
        popTitle.value = `编辑存储服务商 - ${row.name}`;
