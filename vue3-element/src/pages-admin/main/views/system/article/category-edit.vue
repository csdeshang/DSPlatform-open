    ...
    <el-dialog v-model="dialogVisible" :title="popTitle" width="500px" :destroy-on-close="true">
        ...
        <el-form :model="formData" label-width="100px" ref="formRef" :rules="formRules" v-loading="loading">
            ...
            <el-form-item label="分类名称" prop="name">
                <el-input v-model="formData.name" placeholder="请输入文章分类名称"></el-input>
            </el-form-item>
            ...
            <el-form-item label="分类描述" prop="description">
                <el-input v-model="formData.description" :rows="3" type="textarea" placeholder="请输入分类描述" />
            </el-form-item>
...
import { createSysArticleCategory, updateSysArticleCategory, getSysArticleCategoryInfo, getSysArticleCategoryTree } from '@/pages-admin/main/api/system/sysArticle'
...
const fetchMenuTreeOptions = () => {
    ...
    getSysArticleCategoryTree({ }).then(res => {
...
const handleSubmit = async () => {
    ...
    const requestFun = formData.id ? updateSysArticleCategory : createSysArticleCategory
    loading.value = true
...
const setDialogData = async (row: any = null) => {
    ...
    popTitle = '添加分类'
    formData.pid = row?.pid || 0

    if (row?.id) {
        popTitle = '编辑分类'
        const data = await (await getSysArticleCategoryInfo(row.id)).data
