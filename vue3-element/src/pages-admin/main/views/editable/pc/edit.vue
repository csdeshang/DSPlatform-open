<template>
  <div class="flex flex-col h-screen bg-gray-50">
    ...
    <div class="flex justify-between items-center px-5 py-3 bg-white border-b border-gray-200 shadow-sm">
      <h1 class="text-xl font-bold text-gray-800">PC端可视化装修</h1>
...
// 导入组件分类
import pcCategories from './pc-categories.ts';
// 将导入的类别赋值给响应式变量
const componentCategories = ref(pcCategories);
...
const platformModulesFiles = import.meta.glob('@/pages-admin/platform/**/views/editable/pc-components/*.vue', { eager: true });
...
async function fetchPreviewUrl() {
  pcUrl.value = await configStore.fetchConfig('website:pc_url');
  previewUrl.value = new URL('/editable/preview', pcUrl.value).toString();
}
fetchPreviewUrl();
