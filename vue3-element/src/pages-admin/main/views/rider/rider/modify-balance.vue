<template>
    <el-dialog v-model="dialogVisible" :title="popTitle" width="500px" :destroy-on-close="true">
        ...
        <el-form :model="formData" label-width="100px" ref="formRef" :rules="formRules" v-loading="loading"
            class="px-10">
            <el-form-item label="骑手" prop="name">
                <el-text>{{ riderInfo.name }}</el-text>
            </el-form-item>

            <el-form-item label="当前余额">
                <el-text>{{ riderInfo.balance }}</el-text>
            </el-form-item>
...
const handleSubmit = async () => {
    ...
    loading.value = true
    modifyRiderBalance(formData).then(res => {
        loading.value = false
