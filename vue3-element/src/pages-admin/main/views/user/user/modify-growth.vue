    ...
    <el-dialog v-model="dialogVisible" :title="popTitle" width="500px" :destroy-on-close="true">
        ...
        <el-form :model="formData" label-width="100px" ref="formRef" :rules="formRules" v-loading="loading"
            ...
            <el-form-item label="当前成长值">
                <el-text>{{ userInfo.growth }}</el-text>
            </el-form-item>
            ...
            <el-form-item label="调整成长值" prop="change_num">
                <el-input-number v-model="formData.change_num" :min="0" />
            </el-form-item>

            <el-form-item label="调整后成长值">
                <el-text>{{ adjustedGrowth }}</el-text>
            </el-form-item>
...
const formRules = computed(() => {
    return {
        ...
        change_num: [
            { required: true, message: '请输入调整成长值', trigger: 'blur' },
            {
...
const handleSubmit = async () => {
    ...
    loading.value = true
    modifyUserGrowth(formData).then(res => {
        loading.value = false
