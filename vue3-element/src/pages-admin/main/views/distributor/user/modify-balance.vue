    ...
    <el-dialog v-model="dialogVisible" :title="popTitle" width="500px" :destroy-on-close="true">
        ...
        <el-form :model="formData" label-width="100px" ref="formRef" :rules="formRules" v-loading="loading"
            ...
            <el-form-item label="当前余额">
                <el-text>{{ userInfo.distributor_balance }}</el-text>
            </el-form-item>
...
import { modifyDistributorBalance } from '@/pages-admin/main/api/distributor/distributorBalance'
...
const handleSubmit = async () => {
    ...
    loading.value = true
    modifyDistributorBalance(formData).then(res => {
        loading.value = false
...
const setDialogData = async (row: any = null) => {
    ...
    if (row) {
        ...
        formData.distributor_user_id = data.id
    }
    loading.value = false
}
