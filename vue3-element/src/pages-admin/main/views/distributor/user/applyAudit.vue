    ...
    <el-dialog v-model="dialogVisible" :title="popTitle" width="500px" :destroy-on-close="true">
        <el-form :model="formData" label-width="90px" ref="formRef" :rules="formRules"
            ...
            <el-form-item label="审核备注" prop="audit_remark" v-if="formData.apply_status === 2">
                <el-input v-model="formData.audit_remark" type="textarea" :rows="3"
                    placeholder="请输入审核备注" />
            </el-form-item>
        </el-form>
        <template #footer>
...
import { auditDistributorApply } from '@/pages-admin/main/api/distributor/distributorApply';

const dialogVisible = ref(false)
const loading = ref(false)
let popTitle: string = '审核分销商申请'
...
const handleSubmit = async () => {
    ...
    loading.value = true
    auditDistributorApply(formData).then(res => {
        loading.value = false
