    ...
    <el-dialog v-model="dialogVisible" :title="popTitle" width="500px" :destroy-on-close="true">
        ...
        <el-form :model="formData" label-width="100px" ref="formRef" :rules="formRules" v-loading="loading">
            ...
            <el-form-item label="SEO标题" prop="seo_title">
                <el-input v-model="formData.seo_title" placeholder="SEO标题"></el-input>
            </el-form-item>
            <el-form-item label="SEO关键词" prop="seo_keywords">
                <el-input v-model="formData.seo_keywords" placeholder="请输入分类名称"></el-input>
            </el-form-item>
            <el-form-item label="描述" prop="seo_description">
                <el-input v-model="formData.seo_description" :rows="3" type="textarea" placeholder="请输入管理组描述" />
            </el-form-item>
        </el-form>
        <template #footer>
...
import { createTblGoodsCategory, updateTblGoodsCategory, getTblGoodsCategoryInfo, getTblGoodsCategoryTree } from '@/pages-admin/main/api/tbl-goods/tblGoodsCategory'
...
const handleSubmit = async () => {
    ...
    const requestFun = formData.id ? updateTblGoodsCategory : createTblGoodsCategory
    loading.value = true
...
const setDialogData = async (row: any = null) => {
    ...
    if (row.id) {
        popTitle = '编辑分类'
        const data = await (await getTblGoodsCategoryInfo(row.id)).data
