...
class DistributorBalanceLogDao extends BaseDao
{
     ...
     * @return int 受影响的行数
     */
    public function updateDistributorBalanceLog(array $condition, array $data): int
    {
     ...
     * @return array 日志信息
     */
    public function getDistributorBalanceLogInfo(array $condition, string $field = '*', bool $lock = false): array
    {
        return $this->model->where($condition)
            ->field($field)
            ->lock($lock)
     ...
     * @return array 日志信息
     */
    public function getDistributorBalanceLogById(int $id, string $field = '*', bool $lock = false): array
    {
