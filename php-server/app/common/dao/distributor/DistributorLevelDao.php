...
class DistributorLevelDao extends BaseDao
{
     ...
     * @return int 受影响的行数
     */
    public function updateDistributorLevel(array $condition, array $data): int
    {
     ...
     * @return array 等级信息
     */
    public function getDistributorLevelInfo(array $condition, string $field = '*', bool $lock = false): array
    {
        return $this->model->where($condition)
            ->field($field)
            ->lock($lock)
     ...
     * @return array 等级信息
     */
    public function getDistributorLevelById(int $id, string $field = '*', bool $lock = false): array
    {
